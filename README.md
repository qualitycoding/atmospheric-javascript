...
`void-choir.html` contains a copy of `void-choir-engine.js`. The separate file exists so the tests can load the engine, and there is no build script yet, so **if you change the engine, paste the change into the HTML as well**.
...
The suite renders audio offline (via `node-web-audio-api`) and runs 44 checks: same seed gives the same music; guitar harmonic balance is stable across pitch; drum attack, decay and brightness; peak level; the cabinet response at 44.1, 48 and 96 kHz; timing alignment between instruments; and how closely the guitar matches the reference recording.

| Path | Purpose |
|---|---|
| `void-choir.html` | The app (engine inlined) |
| `void-choir-engine.js` | The same engine as a standalone file |
