...
**Try it:** [qualitycoding.github.io/atmospheric-javascript/void-choir.html](https://qualitycoding.github.io/atmospheric-javascript/void-choir.html), or download `void-choir.html` and open it from disk.
...
Example: `void-choir.html?style=blackgaze&seed=glacier`
...
`void-choir.html` is a **generated file**. It is built from two sources: `void-choir.template.html` (the interface) and `void-choir-engine.js` (the audio and music engine). Edit those, then rebuild. Don't edit `void-choir.html` by hand; the next build would overwrite it.

```
npm install          # once, for the test dependency
npm run build        # regenerate void-choir.html from its sources
npm run check        # exit 1 if void-choir.html is out of date
...
The suite renders audio offline (via `node-web-audio-api`) and runs 45 checks: same seed gives the same music; guitar harmonic balance is stable across pitch; drum attack, decay and brightness; peak level; the cabinet response at 44.1, 48 and 96 kHz; timing alignment between instruments; how closely the guitar matches the reference recording; and that `void-choir.html` matches its sources.

| Path | Purpose |
|---|---|
| `void-choir.html` | The app, generated by `build.js` (engine inlined). Serve or open this one |
...
| `build.js` | Generates `void-choir.html` (`--check` verifies it is current) |
